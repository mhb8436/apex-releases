...
APEX analyses Java, JavaScript, HTML, CSS, SQL and XML source against development
...
apex-windows-amd64/
  apex.exe                      ← CLI
  apex-gui.exe                  ← desktop app (Windows and macOS only)
  apex-report.exe               ← DOCX audit report generator (Windows and macOS only)
  configs/
...
The Linux bundle holds only `apex`. Wails needs GTK and WebKit at build time, so we
...
1. Unzip `apex-windows-amd64.zip`
2. Put the folder where you want it (for example `C:\tools\apex\`)
3. Double-click `apex-gui.exe` for the desktop app
4. Optionally add the folder to PATH so the CLI runs from anywhere

```
System Properties → Environment Variables → Path → Edit → add C:\tools\apex
```
...
```bash
tar -xzf apex-darwin-arm64.tar.gz      # on Intel, apex-darwin-amd64.tar.gz
...
> Both front ends are English by default. The app takes its language from Settings; the CLI takes `--lang` (`en` / `ko`) or `APEX_LANG`.
...
4. **DDL cross-analysis** (optional) — point at a DDL file or folder and APEX also
...
**Exporting custom rules** — rules you build in the app live in `~/.apex/custom_rules.yaml`,
...
`apex <path> --profile=custom` runs the same rules in CI.
...
- **Indentation** — new input follows the file you have open. APEX checks tabs
...
Combining profiles can make several rules report the same thing. By default APEX merges findings that land on the same file and line.
...
APEX's rules fall into four kinds, by what they are given and how they match.
...
if "%TARGET%"=="" (
    echo Usage: run_apex.bat [path-to-scan]
    exit /b 1
)

echo === APEX source code scan ===
echo Target: %TARGET%
echo.
...
echo.
echo Report written: apex_report.xlsx
pause
```
...
| `--lang` | - | `en` | output language, `en` or `ko` (or `APEX_LANG`) |
...
With `--offline`, APEX **refuses every connection that leaves loopback (127.0.0.1,
...
|------|------------|
| Choosing the key findings | APEX — by severity and category; the same input always gives the same set |
| File names, line numbers, rule IDs | APEX — straight from the scan; the model cannot change them |
| Quality score and A–E grades | APEX — computed from the debt ratio and the worst security severity |
...
APEX collects files by extension:
