...
APEX는 같은 룰 엔진 위에 데스크톱 앱과 CLI를 얹은 도구입니다. 어느 한쪽이
축소판이 아니고, 둘 중 하나만 골라 쓰라는 구조도 아닙니다.

|  | 데스크톱 앱 (`apex-gui`) | CLI (`apex`) |
|---|---|---|
| 쓰는 자리 | 코드를 훑어보고, 적용할 규칙을 정하고, 넘길 보고서를 만들 때 | CI 파이프라인, 정기 점검, 배치 스크립트 |
...
| 규칙 조정 | 토글 클릭, 정규식 즉석 테스트 | YAML 파일, CLI 옵션 |
...
데스크톱 앱 · 규칙 탭              ruleset.yaml              CI · 매 빌드
 규칙 켜고 끄고 심각도 조정   ──▶   스냅샷 저장   ──▶   apex ./src --overrides ruleset.yaml
```
...
[Releases](https://github.com/mhb8436/apex-releases/releases)에서 내려받습니다.
...
요구하는데 아직 그 빌드를 제공하지 않습니다. Linux에서 APEX는 CLI입니다.

### Windows

압축을 풀고 `apex-gui.exe`를 더블클릭하면 됩니다. 데스크톱 앱은 Microsoft
...
먼저 설치하세요. CLI인 `apex.exe`는 아무것도 필요 없습니다.
...
3. **규칙** (`Ctrl/Cmd+2`) — 이 프로젝트에 안 맞는 규칙을 끄고, 심각도를
   ...
   파일이 이겁니다.
4. **보고서** (`Ctrl/Cmd+4`) — Excel, HTML, JSON으로 내보냅니다. 개발사에
   넘길 때는 보통 Excel을, AI 감리 보고서의 입력으로는 JSON을 씁니다.
5. **AI 감리** (`Ctrl/Cmd+5`) — OpenAI 호환 엔드포인트로 감리 보고서를
...
`apex profiles` 를 치면 지금 쓰는 바이너리 기준으로 목록이 나옵니다.
...
```
apex [경로] [옵션]
apex profiles                       프로파일·그룹 목록
apex report [이름:result.xlsx ...]  Excel 결과로 DOCX 감리 보고서 생성
apex ai-report [scan.json]          점검 JSON으로 AI 감리 보고서 생성
```
...
| 규칙 | `Ctrl/Cmd+2` | 규칙 켜고 끄기, 심각도 변경, 정규식 커스텀 규칙과 즉석 테스트, 스냅샷 저장·불러오기 |
| 결과 | `Ctrl/Cmd+3` | 지표·등급 대시보드, 이슈 목록, 해당 줄로 파일 열기 |
| 보고서 | `Ctrl/Cmd+4` | Excel / HTML / JSON 내보내기, 최근 생성 목록 |
| AI 감리 | `Ctrl/Cmd+5` | 엔드포인트·모델, 연결 확인, 폐쇄망 모드, 진행률, 취소 |
...
APEX는 크래프틱시스템즈 주식회사가 만듭니다. 상용 라이선스나 기술 지원 문의는
이 저장소에 이슈로 남겨 주세요.
